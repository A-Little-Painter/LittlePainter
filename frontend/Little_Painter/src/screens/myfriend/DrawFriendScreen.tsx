...
import {FriendBorder, friendCheckSimilarity} from '../../apis/draw/draw';
...
export default function DrawFriendScreen({
  route,
  navigation,
}: DrawFriendScreenProps) {
  //웹소켓
  ////////////////////////////
  ...
  const handleAnimalBorder = async () => {
    setIsLoading(true);
    try {
      const response = await FriendBorder(personId);
