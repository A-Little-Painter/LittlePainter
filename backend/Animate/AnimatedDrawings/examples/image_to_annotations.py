...
def image_to_annotations(img_fn: str, out_dir: str, animation_type=None) -> None:
    ...
    resp = requests.post("http://172.17.0.2:8080/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    resp = requests.post("http://172.17.0.2:8080/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    if resp is None or resp.status_code >= 300:
    ...
    skeleton = []
    if animation_type=='animals': # 4발동물
        ...
        skeleton.append({'loc' : [round(x) for x in  kpts[6]             ], 'name': 'right_hip_f', 'parent': 'root'})
        skeleton.append({'loc' : [round(x) for x in  kpts[8]             ], 'name': 'right_knee_f'   , 'parent': 'right_shoulder'})
        skeleton.append({'loc' : [round(x) for x in  kpts[10]            ], 'name': 'right_foot_f'    , 'parent': 'right_elbow'})
        skeleton.append({'loc' : [round(x) for x in  kpts[5]             ], 'name': 'left_hip_f' , 'parent': 'root'})
        skeleton.append({'loc' : [round(x) for x in  kpts[7]             ], 'name': 'left_knee_f'    , 'parent': 'left_shoulder'})
        skeleton.append({'loc' : [round(x) for x in  kpts[9]             ], 'name': 'left_foot_f'     , 'parent': 'left_elbow'})
        skeleton.append({'loc' : [round(x) for x in  kpts[12]            ], 'name': 'right_hip_b'     , 'parent': 'root'})
        skeleton.append({'loc' : [round(x) for x in  kpts[14]            ], 'name': 'right_knee_b'    , 'parent': 'right_hip'})
        skeleton.append({'loc' : [round(x) for x in  kpts[16]            ], 'name': 'right_foot_b'    , 'parent': 'right_knee'})
        skeleton.append({'loc' : [round(x) for x in  kpts[11]            ], 'name': 'left_hip_b'      , 'parent': 'root'})
        skeleton.append({'loc' : [round(x) for x in  kpts[13]            ], 'name': 'left_knee_b'     , 'parent': 'left_hip'})
        skeleton.append({'loc' : [round(x) for x in  kpts[15]            ], 'name': 'left_foot_b'     , 'parent': 'left_knee'})
    else: # 인간형
        skeleton.append({'loc': [round(x) for x in (kpts[11] + kpts[12]) / 2], 'name': 'root', 'parent': None})
        skeleton.append({'loc': [round(x) for x in (kpts[11] + kpts[12]) / 2], 'name': 'hip', 'parent': 'root'})
        skeleton.append({'loc': [round(x) for x in (kpts[5] + kpts[6]) / 2], 'name': 'torso', 'parent': 'hip'})
