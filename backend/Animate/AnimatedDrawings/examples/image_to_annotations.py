...
def image_to_annotations(out_dir: str, animation_type=None) -> None:
    ...
    resp = requests.post("http://172.17.0.4:8080/predictions/drawn_humanoid_detector", files=request_data, verify=False)
    ...
    resp = requests.post("http://172.17.0.4:8080/predictions/drawn_humanoid_pose_estimator", files=data_file, verify=False)
    if resp is None or resp.status_code >= 300:
    ...
    if animation_type=='animals': # 4발동물
        ...
        skeleton.append({'loc' : [round(x) for x in  kpts[8]             ], 'name': 'right_knee_f'   , 'parent': 'right_hip_f'})
        skeleton.append({'loc' : [round(x) for x in  kpts[10]            ], 'name': 'right_foot_f'    , 'parent': 'right_knee_f'})
        skeleton.append({'loc' : [round(x) for x in  kpts[5]             ], 'name': 'left_hip_f' , 'parent': 'root'})
        skeleton.append({'loc' : [round(x) for x in  kpts[7]             ], 'name': 'left_knee_f'    , 'parent': 'left_hip_f'})
        skeleton.append({'loc' : [round(x) for x in  kpts[9]             ], 'name': 'left_foot_f'     , 'parent': 'left_knee_f'})
        skeleton.append({'loc' : [round(x) for x in  kpts[12]            ], 'name': 'right_hip_b'     , 'parent': 'root'})
        skeleton.append({'loc' : [round(x) for x in  kpts[14]            ], 'name': 'right_knee_b'    , 'parent': 'right_hip_b'})
        skeleton.append({'loc' : [round(x) for x in  kpts[16]            ], 'name': 'right_foot_b'    , 'parent': 'right_knee_b'})
